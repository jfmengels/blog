...
For lazy nodes, we may or may not have the DOM tree yet under the `node` field:
- This is the initial render, there was no diff
- When diffing, the lazy node was in a sub-tree of a node that was too different from its counterpart

If we don't have it yet, then we compute it and store it. This is the other instance where your lazy function might be evaluated.
...
The current design of the lazy mechanism is one that is hard to use correctly because of the many false negatives when doing the checking, but it is designed for simplicity (no major footguns) and great performance: key checking is fast and the memory footprint is the same as without caching. You can sprinkle it at will with little worries.

Now that we know the limitations and its tradeoffs, I think it's interesting to figure out how we can stretch this in new API additions.

#### Deep equality checks

The major pain point with laziness is the rate of false negatives with the use of JavaScript's
`===`.
If instead we were to use Elm's `==` (that would not crash on function comparison), there would be a lot less cache misses. But the check could have "significant" performance implications in the worst case, for instance if one of the arguments is a very large `Dict` that is only slightly different from the last run.

While this would not be as free as the current mechanism, it could be much easier to use and still be worth it by avoiding an otherwise re-computation of the view function.

#### Remembering more previous runs

I think there could definitely be an addition of lazy nodes that remember more results by storing a list of previous key+results and checking them all. It could be more complicated internally, maybe using a `Dict`, or a `Map`/`WeakMap` but the idea is the same.

There would be more things to check so the cost would increase slightly.
