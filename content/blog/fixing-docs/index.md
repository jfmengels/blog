...
[This rule](https://package.elm-lang.org/packages/jfmengels/elm-review-documentation/latest/Docs-UpToDateReadmeLinks), previously known as `Documentation.ReadmeLinksPointToCurrentVersion`, was the only rule available in v1. I
...
so I renamed it, and that's the only breaking change for this release. I'll re-introduce the rule and its context for those not aware of it.

This is a rule specifically for Elm packages. The way one links to modules from the documentation is by having a Markdown link like
...
documentation on something like GitHub, because then this _ relative_ link will not point to the expected location, which
sparked a nice discussion on the [Elm Discourse](https://discourse.elm-lang.org/t/problems-with-readmes-in-elm-packages/5396/).

The most common solution, and the one suggested in the thread was to use an absolute link to the package registry. So for
links in the README, instead of linking to `[Module-Name](Module-Name)`, you'd link to
...
that links become "too up-to-date", meaning that if you follow a link from the README of an older version of a package (on the registry or GitHub),
you'll be forwarded to a newer version of the package, where the linked section or module may not even exist anymore.
...
or to a different version than the one found in `elm.json`. Sorry, did I say "report"? I meant "fix"!
...
Try it out:
```elm
npx elm-review --template jfmengels/elm-review-documentation/example --rules Docs.UpToDateReadmeLinks
```
...
I ran it on a few published packages, and it already found a number of issues. It also found issues in `elm-review`, though
I believe a part of me didn't spend the time to check for problems with the links because deep down I wanted to write the
rule and find some problems through it, and it did!

A big thanks to [`lue`](https://github.com/lue-bird) which spear-headed this work (and the focus on the rest of the package)
and solved the problems that blocked me!

Try it out:
```elm
npx elm-review --template jfmengels/elm-review-documentation/example --rules Docs.ReviewLinksAndSections
```
...
This is in a way a blueprint of how the module's documentation will look like once published:
...
Unfortunately, I have seen packages where the documentation was pretty broken because `@docs` was slightly misused, in a
...
Try it out:
```elm
npx elm-review --template jfmengels/elm-review-documentation/example --rules Docs.ReviewAtDocs
```
...
But in that style, I see quite a few authors assume that the documentation is read on the package registry, from top to
bottom, meaning that redundant functions are not always described when they feel redundant in their context. For instance,
if you've just described `map2`, then you may not be inclined to describe `map3`.

But this way of accessing the documentation is not the only one. A number of developers access the documentation through
their IDE, where when you try to access the empty documentation for `map3`, all you'll get is an empty and unhelpful
...
`{ document = everything, from = onlyExposedModules }` would be the equivalent of what the Elm compiler reports, but you
can go a step further with the other options.

Try it out:

```elm
npx elm-review --template jfmengels/elm-review-documentation/example --rules Docs.NoMissing
```
...
But that's not the case for **application** projects. We all know that documentation quickly gets out of date, so
application developers tend to write little of it even when it would be really valuable.
...
any hint or warning you get that something is wrong or out of date can help prevent outdatedness.
These will not fix everything, but if you stop exposing a function, and you're automatically asked to remove its `@docs`
...
and a chore. But if you make it good, then people will use it more, improving it bit by bit and so on. It's either going
to be a vicious cycle or a virtuous cycle.

I hope to give you the tools to start creating a new habit that leans toward a virtuous cycle. Did you know you can
already visualize your application's documentation using
[`elm-doc-preview`](https://github.com/dmy/elm-doc-preview#applications-support)? Well, now you know, and maybe you'll
share that knowledge I recently came across to others. Then maybe people will feel like it can be improved somehow, and
extend it so that it generates interactive examples from the module documentation. Who knows? I'm looking forward to see
what people come up.

TODO NoMissing is missing some configuration options. Ask feedback

## Afterword



TODO I hope there will be more and better tools to preview documentation for applications.
TODO I hope some of these changes get into the Elm compiler (@docs, dead links, ...)
TODO Talk to your company about sponsoring me
