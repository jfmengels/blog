...
but it will likely remain mostly unchanged, except for options like `"files"`, `"plugins"` and `"rules"`, which we'll focus on.
Maybe they need to be rethought as well, but I have nothing to bring to the table on that subject.
...
be the better option for better backwards compatibility and consistency with the current naming convention. If you don't mind, I'll stick with the camel case for now though.
...
it happens. If someone types `react.frbidPropTypes()`, there will be stack trace that hopefully will be enough to figure
out the issue.

I think that the current behavior of reporting an error in every file is sub-par, though I guess there are some reasons
for that I'm aware of. 

Overall, learning to configure `ESLint` will be a lot closer to learning JavaScript than learning a new tool, just like when we filter rules or concatenate rule arrays.
...
for disable comments at the top of a file, and move all the knowledge of what is ignored and disabled to the configuration
file(s).
...
There could also be a `.onlyInDirectories([...])` and similar variants, which `elm-review` has not felt the need for yet. Someone would have to choose what happens when both `onlyIn...` and `ignoreIn...` variants are used together in contradictory ways though.
...
- I believe that the configuration of rules is a lot more straightforward like this, compared to the complex merge system that needs to be taught to users and the magic numbers/strings (`2`/`"error"`).
